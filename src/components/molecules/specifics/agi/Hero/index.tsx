    ...
    >
          ...
          <span className="flex flex-row gap-2 md:w-[40%] md:justify-end items-center h-full">
            {breadcrumbsData.map((item, index) => (
              <React.Fragment key={index}>
                <Link
                  href={item.href}
                  className={`font-opensans text-white text-[1.125rem] ${index === breadcrumbsData.length - 1 ? 'font-bold cursor-default pointer-events-none' : ''}`}
                >
                  {item.title}
                </Link>
      ...
      {bottomImage && (
        <div className="-z-[1] w-full top-[6.25rem] sm:top-[12.5rem] absolute">
          <Image
            className="w-full h-[20rem] md:h-[40rem] object-fill"
